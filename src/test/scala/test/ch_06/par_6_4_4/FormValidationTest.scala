...
class FormValidationTest extends FunSuite {
  ...
  object EitherOnly {
    ...
    assert(parseInt("foo") == Left(List(s"""must be an integer java.lang.NumberFormatException: For input string: "foo"""")))

    def nonNegative(data: Int): FailFast[Int] = Right[List[String], Int](data).
      ensure(List(s"must be non-negative"))(_ >= 0)
    ...
    def readName(m: Map[String, String]): FailFast[String] = for {
      n <- getValue(m, "name")
      nb <- nonBlank(n)
    } yield nb


    def readAge(m: Map[String, String]): FailFast[Int] = for {
      n <- getValue(m, "age")
      i <- parseInt(n)
      nb <- nonNegative(i)
    } yield nb
